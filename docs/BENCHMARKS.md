...
## RP2350 / Pico 2 results (measured 2026-07-18, nightly dev2026071006)
...
3. **The nested-loop kernels are a real gap on this path.** CRC-32 runs at
   1.88× and matmul at 2.09× of clang C (quicksort 1.37×) — clearly larger
...
5. Firmware `.text`: Mojo 4,196 B — the smallest of the four (clang
   4,590 B, gcc 4,902 B, Rust 14,064 B, which drags in its own intrinsics).
...
## Language-feature measurements, RP2350 (measured 2026-07-18, nightly dev2026071006)
...
| comptime-materialized LUT | 26.11 | recomputing inline | 12.04 |
| `comptime for` unrolled ×16 | 63.13 | runtime loop ×16 | 105.05 |
| `@no_inline` call | 9.05 | `@always_inline` | ~0 (loop strength-reduced) |
...
- **A comptime lookup table is not free on an XIP part**: the table lives in
  ...
  registers — the LUT loses by ~2.2×. Measure before caching.
...
|---|---:|
| 8 B `TrivialRegisterPassable` | 10.03 |
| 8 B plain struct (owned) | 10.05 |
| 16 B plain struct | 12.03 |
| 32 B plain struct | 16.05 |
| 64 B plain struct | 26.07 |
| returning a 16 B struct | 19.05 |
