...
def init():
    ...
    write32(CLK_ADC_CTRL, CLK_ENABLE | CLK_ADC_AUXSRC_XOSC)
    write32_clr(RESETS_RESET, RESET_ADC)
    while (read32(RESETS_RESET_DONE) & RESET_ADC) == 0:
        pass
    write32(ADC_CS, ADC_CS_EN)
    while (read32(ADC_CS) & ADC_CS_READY) == 0:
        pass


def read(channel: UInt32) -> UInt32:
    ...
    while (read32(ADC_CS) & ADC_CS_READY) == 0:
        pass
    write32_set(ADC_CS, ADC_CS_START_ONCE)
    while (read32(ADC_CS) & ADC_CS_READY) == 0:
        pass
    return read32(ADC_RESULT) & 0xFFF


def read_temp_milli_c() -> Int32:
