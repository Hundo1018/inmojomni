...
Usage: pixi run chart  [in.csv out.svg ["subtitle"]]
(defaults render the RP2040 run; the RP2350 driver's csv works the same,
pass a subtitle naming the chip/clock/timebase)
"""
...
def render(rows: List[Row], subtitle: String, src: String) -> String:
    ...
    s += _text(20, height - 14, 10, String("#8b949e"), String("start"),
               String("400"),
               String("Generated by `pixi run chart` from ") + src)
    s += "</svg>\n"
    return s^
...
def main() raises:
    ...
    var subtitle = String(
        "RP2040 @ 12 MHz &#183; medians of 3 runs &#183; "
